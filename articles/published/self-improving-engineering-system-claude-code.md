---
title: "I Built a Self-Improving Engineering System for Claude Code"
...
This guide walks through all fifteen moving pieces of the system. I wrote it so that even if you've never touched Claude Code -- or if you're a product manager trying to understand how your team could use this -- you'll walk away with the full picture.
...
The system has several hooks, each triggered at a different moment:
...
- **After every file edit:** Auto-format the code. The agent never needs to remember formatting rules -- every edit is cleaned up automatically.
- **When the agent tries to stop working:** Type-check the TypeScript project. If there are type errors, the agent can't stop -- it has to fix them first.
...
> Deep dive: [settings.json](https://github.com/vinicius91carvalho/.claude/blob/main/settings.json) | [hooks/](https://github.com/vinicius91carvalho/.claude/tree/main/hooks) -- [block-dangerous.sh](https://github.com/vinicius91carvalho/.claude/blob/main/hooks/block-dangerous.sh), [post-edit-quality.sh](https://github.com/vinicius91carvalho/.claude/blob/main/hooks/post-edit-quality.sh), [end-of-turn-typecheck.sh](https://github.com/vinicius91carvalho/.claude/blob/main/hooks/end-of-turn-typecheck.sh), [compound-reminder.sh](https://github.com/vinicius91carvalho/.claude/blob/main/hooks/compound-reminder.sh)
...
Five skills (think: automated procedures) form the complete pipeline:
...
The preferred flow is mostly autonomous: I review the plan once, approve it, the system builds and tests everything locally, I do a manual check, and then the deployment pipeline takes over -- creating a branch, opening a pull request, running tests on staging, and deploying. The one place it always stops and asks? **Production deploy.** That gate is non-negotiable.
...
It's opinionated. Enforces pnpm, enforces TDD, blocks force pushes. You might disagree with some of those choices. Good. Fork it and change them. An engineering system without opinions is just a folder with markdown files.
